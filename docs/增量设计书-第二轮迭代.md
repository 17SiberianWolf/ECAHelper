...
**边界（team-lead 已裁决，写死）**：
- **前端为主**：改动主体落在 `web/static/**` 与 `web/templates/**`。
- **后端改动恰好 3 处，不得有第 4 处**（team-lead 裁决 CLI-Q1）：
  ...
  3. **唯一一处「行为语义」改动**：`aggregate._base_where`（**L65–66**）把 `task` 由 `=` 改为 `LIKE '%kw%'`（**无 task 条件时 SQL 完全不变** → 不影响红线守恒）。
  > 除上述 3 处外，**批次 1 不得再有任何后端/口径/DB 改动**。
- **绝不触碰**：项目号归并（`PROJECT_BASE_SQL`）、导出列、`schema.sql`、日志模块。

> ⚠️ 边界说明：任务字段的「包含匹配」属批次 1 功能（US-R2-03）之必需，无法纯前端实现，故为批次 1 仅有的后端语义改动；它只影响「带 task 关键字查询时命中哪些行」，不改变任何全量合计。**「批次 1 零后端」并非 team-lead 的要求**（详见 §10 Q1）。
...
| 2 | `src/eca_helper/queries/aggregate.py` | 新增模块级常量 `PROJECT_BASE_SQL`；按 **§5.2 的 10 处**逐处替换/改写（含 `_base_where`(L71) 的 `project_id`、`_DIM_SQL["project"]`、`aggregate_project`、`wbs_drill`、`aggregate_employee`、`search`、`list_projects`、`project_name_for`）；§5.2.1 的 5 处 NULL 判定/原始行展示**有意保留原列**；新增 `project_children(conn, base, start, end)`（子项目下钻）。 |
| 2 | `src/eca_helper/queries/export.py` | `fetch_rows`（**动态拼接，无字面量**）改为按列定制：`project_id_norm` 取 `PROJECT_BASE_SQL AS project_id_norm`（父号主列）、`project_id_raw` 取 `r.project_id_raw`（原始项目号，既有列，**不新增 DB 列**）；顶部新增 `from …aggregate import PROJECT_BASE_SQL`（SQL 片段见 §5.4）。 |
| 2 | `src/eca_helper/routes/chart_routes.py` | 人员×项目矩阵 **L112 SELECT / L115 IN / L116 GROUP BY 三处同步**改用 `PROJECT_BASE_SQL`（保证与 `top_n(project)` 同口径，避免 `cells` 键不一致导致热力图全空）。 |
...
> 共 **10 处**（team-lead 独立核对后补齐；原稿 9 项遗漏 `_base_where`、`chart_routes` 不完整、`export` 机制描述失真）。除本清单 + §5.2.1「有意保留原列」外，**不得**再改任何 `project_id_norm` 引用。

| # | 位置（函数 / 行） | 现状 | 改为 |
|---|---|---|---|
| 1 | `_base_where`（**L71**，`if f.project_id:` 分支） | `conds.append("r.project_id_norm = :project_id")` | `conds.append(PROJECT_BASE_SQL + " = :project_id")`。**不加前导 `AND`**——该函数末尾已是 `return (" AND " + " AND ".join(conds))`，加 `" AND "` 会拼出双 `AND` 语法错误。被 **`search()`、`top_n(extra)`、`export.fetch_rows`、`chart_routes._month_series`** 四处共用 → 不改则「按项目筛选 / 导出父项目 / 趋势图」**全漏子行**（三层结果有数据、趋势图却空白） |
| 2 | `_DIM_SQL["project"]`（L232） | `"r.project_id_norm"` | `PROJECT_BASE_SQL` |
| 3 | `aggregate_project` where（L129） | `" AND r.project_id_norm = :pid AND …"` | `" AND " + PROJECT_BASE_SQL + " = :pid AND …"`（查询父号 → 匹配整棵子树） |
| 4 | `wbs_drill`（L149） | `"WHERE r.project_id_norm = :pid "` | `"WHERE " + PROJECT_BASE_SQL + " = :pid "`（**必须按父号匹配子树**） |
| 5 | `aggregate_employee` 按项目（L179 select / L182 cond / L183 group） | `r.project_id_norm AS key` / `… IS NOT NULL` / `GROUP BY r.project_id_norm` | `PROJECT_BASE_SQL AS key` / 条件**保留**（NULL 判定） / `GROUP BY " + PROJECT_BASE_SQL` |
| 6 | `search` 按项目（L216 select / L219 group） | `r.project_id_norm AS key` / `GROUP BY r.project_id_norm` | `PROJECT_BASE_SQL AS key` / `GROUP BY " + PROJECT_BASE_SQL` |
| 7 | `list_projects`（L306；`WHERE … IS NOT NULL` **保留**，仅 `GROUP BY` 改） | `GROUP BY project_id_norm` | `GROUP BY " + PROJECT_BASE_SQL`（另增 `suspicious`，见 §5.6） |
| 8 | `project_name_for`（L279 where） | `WHERE project_id_norm = ?` | `WHERE " + PROJECT_BASE_SQL + " = ?`（父号取代表名） |
| 9 | `chart_routes.api_chart_matrix`（**L112 SELECT / L115 IN / L116 GROUP BY**） | `"SELECT r.project_id_norm AS pid, …"` / `r.project_id_norm IN (…)` / `GROUP BY r.project_id_norm, r.resource_id_norm` | `"SELECT " + PROJECT_BASE_SQL + " AS pid, …"` / `PROJECT_BASE_SQL IN (…)` / `GROUP BY " + PROJECT_BASE_SQL + ", r.resource_id_norm`。**L112 必须与 L115/L116 同步**：`cells` 以 `f"{pid}\|{rid}"` 为键，而前端用 `top_n(project)`（已父号）查表，SELECT 不同步 → **热力图全部取不到值** |
| 10 | `export.fetch_rows`（L44–48，**动态拼接，无字面量可替换**） | `"SELECT " + ", ".join(f"r.{c}" for c in cols)` | 改为**按列定制**：`project_id_norm` → `PROJECT_BASE_SQL + " AS project_id_norm"`；`project_id_raw` → `"r.project_id_raw AS project_id_raw"`；其余 → `f"r.{c}"`（**可直接照抄的 SQL 片段见 §5.4**） |

> 替换后：下拉项数 1,552 → **1,262**（AC-02-1）；总工时**守恒 1,474,383.8 h**（AC-02-2）；父 = 各子之和（AC-02-3）；`0.0001 / OE1323.172 / 70463 / 单点号值` 不被改写（AC-02-6）。

#### 5.2.1 有意保留原列（**禁止误改**）

以下位置**保持 `r.project_id_norm` 原列不动**，属**有意为之**，工程师**不得**替换为 `PROJECT_BASE_SQL`：

| 位置（函数 / 行） | 用途 | 保留理由 |
|---|---|---|
| `_empty_project_cond`（**L79–80**） | `AND r.project_id_norm IS NOT NULL`（项目维度是否纳入空项目号行） | **NULL 判定**；`PROJECT_BASE_SQL` 对 NULL 输入走 `ELSE` 分支返回 NULL，语义等价，保留原列更省 |
| `aggregate_employee` 非项目工时（**L192**） | `AND r.project_id_norm IS NULL`（非项目工时按 task 细分） | 同上，NULL 判定 |
| `list_projects`（**L306**） | `WHERE project_id_norm IS NOT NULL` | 同上；**仅其 `GROUP BY` 改**（见 §5.2#7） |
| `quality_service.quality_summary`（**L22**） | `SELECT COUNT(*) … WHERE project_id_norm IS NULL`（空项目号行计数） | 同上，NULL 计数 |
| `quality_service.detail_rows`（**L127**） | `r.project_id_norm AS project_id_norm`（质量明细行展示） | **审计源码行**：**有意保留原始子项目号**，便于逐行溯源到 Excel |

> 判据：**凡 `IS NULL` / `IS NOT NULL` 的 NULL 判定、以及质量面板的「原始行展示」，一律保留 `r.project_id_norm`**；凡**分组或把“项目号”作为口径展示**的地方，才用 `PROJECT_BASE_SQL`。
...
- `config.BILINGUAL` 增：
  ...
  ```
- `export.py` 顶部导入新增 `PROJECT_BASE_SQL`：
  ```python
  from eca_helper.queries.aggregate import (
      EXCLUSION_SQL, PROJECT_BASE_SQL, QueryFilter, _base_where, _empty_project_cond)
  ```
- `export.fetch_rows`（L39–54）**没有字面量可替换**（L45 是 `"SELECT " + ", ".join(f"r.{c}" for c in cols)` 动态拼接），须改为**按列定制**。**可直接照抄**（`where`/`params`/`ORDER BY`/排除条款/`task_zh` 均不变）：
  ```python
  def fetch_rows(conn, f: QueryFilter) -> list[dict]:
      ...
      for c in _select_columns():                      # _select_columns() 已剔除派生列 task_zh
          if c == "project_id_norm":
              exprs.append(PROJECT_BASE_SQL + " AS project_id_norm")    # 父号主列（归并口径）
          elif c == "project_id_raw":
              exprs.append("r.project_id_raw AS project_id_raw")        # 原始项目号（既有列）
          else:
              exprs.append(f"r.{c}")
      sql = (
          "SELECT " + ", ".join(exprs)
      ...
      return rows
  ```
- **关键点**：父号列 `project_id_norm` 与原始号列 `project_id_raw` **同一行并存**（决策 9）；`_row_values()`（L67–68 按 `EXPORT_COLUMNS` 取值）无需改动，两个键均已存在。
- **不新增 DB 列**（`project_id_raw` 是 `timesheet_record` 既有列），同一行两列并存（决策 9 / Q-F）。
...
flowchart TD
  ...
  B --> E["_base_where(L71) project_id 过滤 → search / top_n(extra) / 导出 / 趋势图"]
  B --> M["search.by_project / aggregate_employee.by_project"]
  B --> F["aggregate_project 主查询(+by_month/by_person)"]
  B --> G["chart_routes.matrix(人员×项目) L112/L115/L116"]
  B --> H["export.fetch_rows<br/>父号主列(按列定制)"]
  A --> I["project_id_raw → 导出「原始项目号」列(不新增DB列)"]
  F --> J["wbs_drill / project_children<br/>按父号匹配子树"]
  J --> K["子项目下钻 / WBS 折叠区块"]
  C --> L["可疑徽标(日期型项目号)"]
  N["有意保留原列(§5.2.1)<br/>IS NULL / IS NOT NULL / 质量原始行"] -.禁改.-> A
```
...
- **仅落本机、零远程上报**：文件日志 → `DATA_DIR/logs/`；审计 → 本机 SQLite；**不创建任何网络/远程 handler，不做任何上报/同步**（决策 6 / AC-07-5）。
- **员工姓名不脱敏（team-lead 裁决，原样保留）**：审计表 `target`/`detail` **保留 `resource_id_norm` 与实际员工姓名**。理由：本机单人应用，同一 `eca_helper.db` 内本就存有全部姓名与工时，脱敏不降低任何**实际**暴露面，却会让「谁查了什么」这一审计核心问题失去答案 → 故**按原文记录**。
- **补偿性提示**：仅在**日志页面**与**CSV 导出**加一行提示——`ⓘ 含员工姓名，仅限本机使用，请勿外发`；数据库文件建议保持在本机数据目录（见 `docs/部署手册.md`）。
- **默认裁决**：Q4 已裁决为「**不脱敏**」（见 §10）。
...
| 任务 | 名称 | 涉及文件 | 依赖 | 完成判据 |
|---|---|---|---|---|
| **T05** | 项目号归并数据层 | `src/eca_helper/queries/aggregate.py`、`src/eca_helper/queries/export.py`、`src/eca_helper/routes/chart_routes.py`、`src/config.py` | T01（下拉展示父号）、T02 | `PROJECT_BASE_SQL` 唯一定义并在 **§5.2 的 10 处**替换到位（§5.2.1 的 5 处**有意保留**）；下拉项 1,552→**1,262**；导出「项目号」= 父号 + 新增「原始项目号」列（双语表头）；矩阵与 `top_n` 同口径。**硬指标①：全量 33,015 行 / 1,572,771.8 h 守恒；归并口径 1,474,383.8 h 守恒**。 |
...
> **7 项均已经 team-lead 裁决**（下方「裁决」列为最终结论；原「推荐默认值」的歧义已消除）。

| # | 事项 | 裁决（team-lead） | 影响面 |
|---|---|---|---|
| **Q1** | **批次 1 的后端边界** | **已批准**：批次 1 后端改动**恰好 3 处**（① `GET /api/options/tasks` 只读 ② `GET /api/import/candidates` 只读 ③ `_base_where` 的 `task` 改包含匹配）。**不得有第 4 处**；team-lead 从未要求「批次 1 零后端」。已写死进 §1.3 | T02 可独立交付 |
| **Q2** | **导航「系统日志」入口归属** | **已批准**：**批次 2 加**（页面同批）；批次 1 的 `base.html` 只加 `components.js` 引入 | 批次 1 提交最小化 |
| **Q3** | **任务候选是否套用 `EXCLUSION_SQL`** | **已批准**：套用（默认 exclusion 表为空，结果相同） | 极小 |
| **Q4** | **审计表员工姓名是否脱敏** | **已裁决：不脱敏，保留原文**（保留 `resource_id_norm` 与实际姓名）。理由见 §6.4；仅在日志页/CSV 加「含员工姓名，仅限本机使用」提示；**保持零远程上报** | 隐私边界 |
| **Q5** | **日志页时间范围控件形态** | **已批准**：复用 `YearMonth`（年+月） | 交互一致性 |
| **Q6** | **全站 `<input type="month">` 替换面** | **已批准**：共 3 处（`project_query`/`employee_query`/`search`），T04 复核 | T02 完整性 |
| **Q7** | **任务下拉「（不限）」空选项** | **已批准**：保留，置首位，值为 `""` | 交互细节 |
...
*参考：`docs/增量PRD-第二轮迭代.md`（本轮需求）、`docs/系统设计书.md`（既有接口/蓝图基线）、`docs/附录A-源数据实测报告.md`（44 文件解析基线）。*
