...
**对账锚点（回归红线）**：行 **33,015**、总工时 **1,572,771.8 h**、空项目号 **6,575**、重复文件 **29**、异常 **14**、0 工时 **36**、Resource ID **270**、含别名变体 **192**。
...
| **空库自举不阻塞服务** | 首启需导入 43 文件（数十秒），但不能卡死 Web 服务/黑窗 | `ensure_db()` 后**后台守护线程**执行自动导入；`waitress` 立即起服务 |
