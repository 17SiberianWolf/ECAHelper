...
> 输入基线：`docs/PRD.md`（许清楚）、`docs/附录A-源数据实测报告.md`（齐活林，44 文件全量解析）
> 读者：工程师 寇豆码（下一步批量写代码依据）

> ⚠️ **实现变更说明**：本文为初版系统设计。图表库决策已变更——因离线沙箱无法下载 ECharts，**实际实现采用原生 SVG 离线渲染**（`web/static/js/charts.js`，无任何 CDN / 第三方图表库），覆盖 Top N 排行 / 按月趋势 / 人员×项目交叉矩阵。全站无 `echarts` 依赖、无 vendored 文件。其余架构（Flask + Waitress + SQLite + 原生 JS/CSS）不变。详见 `docs/增量设计书-第二轮迭代.md` 与 `docs/设计书总览.md` §3/§4。
...
| 图表 | **原生 SVG（自研 `charts.js`，无第三方库）** | 单 JS 文件，覆盖 Top N 排行 / 按月趋势 / 人员×项目交叉矩阵（heatmap），完全离线、无 CDN |
...
| 维度 | Vite+React+MUI+Tailwind（前端 SPA） | **Flask+Jinja2+原生 SVG（本方案）** |
|------|--------------------------------------|--------------------------------------|
| 运行时 | Python(后端API) + Node(前端构建) 双工具链 | 仅 Python |
| 交付复杂度 | 需 `npm install` + `npm run build` 再 serve dist，两步部署 | 单 `.bat` 起 Python 即服务，零构建 |
| 离线 | 构建产物可离线，但构建期需 Node/网络 | 全离线（原生 SVG，无第三方图表库） |
| 交互能力 | 组件化、状态管理强 | 折叠/筛选/图表用原生 JS 足够（报表型工具） |
| 风险 | 引入 Node 工具链，本地单人工具收益低、风险高 | 最省事、最易交付、最稳 |

**结论**：本场景是「单人本地 + 二维表格 + 图表」的报表工具，没有复杂客户端状态。引入 React 构建链收益极低、交付风险高。**明确采用 Flask + 服务端渲染 + 原生 SVG 图表**，不引入 Node。API 层保持干净（JSON 接口），若二期要换 SPA 可无痛替换前端。
...
| 路径 | 职责 |
|------|------|
| `templates/base.html` | 布局、导航、引入原生 SVG 图表(charts.js)/样式 |
...
| `static/js/charts.js` | 原生 SVG 图表渲染（无第三方库） |
...
| T11 | 图表（Top N / 趋势 / 交叉矩阵） | T07,T10 | P1/P2 | `static/js/charts.js`, `chart_routes.py` | Top N（维度/N 可配）；按月趋势（缺月补 0）；人员×项目矩阵（合计一致）；离线可用 |
...
| 图表 | 原生 SVG（自研，无第三方库） | 前端图表渲染 | 否（实现于 `static/js/charts.js`） |
...
> 以下均给出技术侧建议，**已于 2026-09-21 全部确认，取值与设计书总览 §4 一致**：Q1 空ID→`__UNKNOWN__`；Q2 默认不自动合并（WATA/WATA02 为不同人，合并候选须保守）；Q3 默认不排除、可勾选持久化；Q4 同名重导默认跳过；Q5 单季=当前自然季；Q6 仅提示不归一；Q7 双语含可选业务值映射；Q8 0 工时计入行数；+1 原生 SVG 离线渲染（无 ECharts/CDN）；+2 端口 5000+1；+3 导出命名规则。全部采用技术侧建议。
...
| +1 | 图表库 | 初版推荐 ECharts 单文件 vendored；**实现因离线沙箱无法下载，已改为原生 SVG 离线渲染（`charts.js`，无第三方库）**，覆盖排行/趋势/矩阵。**建议原生 SVG** |
