...
| `OriginSource/` | 输入 xlsx（已存在，43 有效表 + 1 主数据 + 1 锁文件） |
...
- 实测：29 文件存在重复，单文件 1~7 组，绝大多数重复 2 次。
...
- 0 工时行 = 36 行（均为 0，无负工时），计入行数、合计 0。
...
| T04 | 归一化与解析基础 | T01 | P0 | `parsers/normalizer.py` | 5 种 project 形态→可比 key；43 文件名月份全过（含 `2025.01including`）；空 ID→`__UNKNOWN__` |
| T05 | 导入管线（批量/幂等/去重/异常） | T02,T03,T04 | P0 | `parsers/importer.py`, `import_routes.py` | 导 43 文件→33,015 行；重复 29 文件标记；异常 14 行(2023.2)；同名重导默认 skip 不翻倍；任意行可 `source_file+source_row` 回溯 |
| T06 | 人员维度与合并服务 | T02,T04,T05 | P0/P2 | `services/person_service.py`, `person_routes`(并入各 query 路由) | 270 ID 唯一；别名聚合正确；任一别名可命中；合并后聚合重路由（P2 默认不自动） |
...
| T08 | 数据质量面板服务 | T05,T07 | P0 | `services/quality_service.py`, `quality_routes.py` | 面板数字与库一致（空 PID 6575/重复29/异常14/0工时36/变体192-270）；可下钻 `source_row`；勾选排除持久化 |
| T09 | 导入页 UI | T05,T08 | P0 | `templates/import.html`, `static/js/app.js`(导入部分) | 选目录/文件→预览 43 文件+区间；开始导入→报告（新增行/跳过/标记）；跳质量面板 |
...
| Q8 | 0 工时行（36 行）计入？ | 保留并计入行数，工时 0 不影响合计；质量面板单列口径。**建议计入行数、合计 0** |
